...
# Ensure .bat file is in windows startup folder
# Hidden console based version; Saves and organizes images from 4chan with filter
# Saves output to .txt and will display in the console section 

# Before starting goto 4chan.org and click accept

WaitTime = 360 # Seconds
...
# Reads the text file for destination. If invalid writes error to console.
def GetDestination():
    with open("destination.txt", "r+") as f: 
        destination = f.read()
        if os.path.isabs(destination) and not os.path.exists(destination):
            os.makedirs(destination)
        elif not os.path.isdir(destination):
            with open("console.txt", "a+") as f:
                f.write("\n" + (datetime.now().strftime("%H:%M") + " | [ERROR] Destination is invalid "))   
                f.close() 
                exit()
...
while True:
    for selection in GetSelections():
        try:
            imageSaver(selection, GetDestination())
        except requests.exceptions.SSLError:
            with open("console.txt", "a+") as f:
                f.write("\n" + (datetime.now().strftime("%H:%M") + " | [ERROR] Silent could not connect "))   
                f.close() 
                exit()
